...
knitr::read_chunk(here("analysis","03_Duncan_rings-to-pith.R"))
```
...
$$
\begin{aligned}
\text{log}(d_i) &\sim N(\mu_{\text{tree}[i]}, \sigma_d) \\
\mu_{\text{tree}[i]} &= \beta_{\text{patch}[i]} + b_{\text{tree}[i]} \\
...
Normal QQ plot of tree-level intercepts, grouped by patch [currently broken because of some mismatch between trees in `inner_rings` and trees in `duncan`]...
...
```{r duncan_rings-to-pith_joyplots, echo=FALSE, message=FALSE, fig.width=7, fig.height=7}
